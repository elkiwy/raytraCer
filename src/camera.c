...
camera* make_camera(float3 from, float3 at, float3 vup, float vfov, float aspect_ratio, float aperture, float focus_dist) {
    ...
    cl_float3 cl_from = {{from.x, from.y, from.z}};
    cl_float3 cl_at = {{at.x, at.y, at.z}};
    cl_float3 cl_vup = {{vup.x, vup.y, vup.z}};
    init_camera(c, cl_from, cl_at, cl_vup, vfov, aspect_ratio, aperture, focus_dist);
    return c;
}
